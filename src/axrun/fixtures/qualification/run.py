"""Fail-closed task image and optional Claude mount qualification."""
...
def main() -> int:
    ...
    if args.claude:
        ...
        if manifest.get("canonical_mount_target") != "/__claude_code":
            raise RuntimeError("Claude manifest mount target mismatch")
        if manifest.get("canonical_entry") != "/__claude_code/usr/local/bin/claude":
            raise RuntimeError("Claude manifest entry mismatch")
        claude_version = _output([str(entry), "--version"])
        node_version = _output([str(node), "--version"])
        if "2.1.205 (Claude Code)" not in claude_version:
            raise RuntimeError("Claude Code version mismatch")
        if node_version != "v22.23.2":
            raise RuntimeError("Node.js version mismatch")
        payload["claude"] = {
            "entry": str(entry),
            "mount_readonly": True,
